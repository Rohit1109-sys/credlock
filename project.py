...
def login_window(app: App):
    ...
    # Right side form
    form_frame = ctk.CTkFrame(login, width=500, height=550, corner_radius=15, fg_color="white")
    form_frame.place(x=600, y=80)

    title_label = ctk.CTkLabel(form_frame, text="Enter Your PIN", font=("Arial", 18, "bold"))
    title_label.pack(pady=10)

    subtitle_label = ctk.CTkLabel(form_frame, text="Enter your PIN to start with Credlock!", wraplength=250)
    ...
    for i in range(4):
        entry = ctk.CTkEntry(frame, textvariable=pin_vars[i], width=40, justify="center", font=("Arial", 20), show="*")
...
if __name__ == "__main__":
    play_video()  # Show splash video first
